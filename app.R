...
server <- function(input, output, session) {
  ...
  filtered_data <- reactive ({
    ...
    # disclaimer: I had a lot of problems retreiving data from dynamically 
...
### things for kaplan meier curve, needs to be moved
# my_data <- my_data %>% 
#   mutate("Days_to_Event" = difftime(DATA_UC, DATA_DIAG, units=c("days")))
...
#   )
# my_data <- my_data %>%
#   mutate_at(c("GRAUDIF"), list(~replace(., . %in% c(0,4,8,9), NA)))
# tagList(
#   column(3,
...
# )
##### GLOBALS #####
# CATEGORIC <- c("GRAUDIF", "T", "N", "M", "SUBTYPE", "ESTADI2")
...
#
# for making a plot resizable
# alert! this js property messes up ggplot parameters
...
# my_data <- reactive({
#   req(input$inputExcel)
#   dt <- read_excel(input$inputExcel$datapath)
#   dt
# })
# 
# ##### Format Data #####
# # reads colnames introduced as Continuos and Categoric values lists
# # and transform them into numeric and factor values respectively
# 
# formated_data <-reactive({
#   req(my_data(), Categoric(), Continuos())
#   formated_data <- my_data()
#   formated_data <- to.numeric(formated_data, Continuos())
#   formated_data <- to.factor(formated_data, Categoric())
#   formated_data
# })